...
import '../../models/task_search_model/task_search_model.dart';
import '../../utils/helpers/validation.dart';

class PunchInOutBottomSheetController extends GetxController
    with SnackbarMixin {
  ...
  final _taskSearch = (List<TaskSearchModel>.empty()).obs;
  List<TaskSearchModel> get taskSearch => _taskSearch;
    ...
    try {
      ...
      if (decodedToken != null) {
        ...
        if (response.status == 200) {
          ...
          List<String> projectNames = projectSearch.map((project) => project.projectName).toList();
          print(projectNames); // This will print the list of project names
        }
      }
    } catch (e) {
      showErrorSnackbar(message: e.toString());
  ...
  getTaskSearch() async {
    try {
      ...
      if (decodedToken != null) {
        ...
        final request = TaskSearchRequest(
          field: "",
          sortOfOrder: "",
          page: 0,
          size: 4,
          keyword: '',
          projectId: "0bdba80d-cb0a-415f-b269-0383aa6a65ee",
          userId: userId,
          isArchived: false,
          isDeleted: false,
        );

        final response = await ApiRepository.to.taskSearch(request: request);
        if (response.status == 200) {
          _taskSearch.value = response.data;

          List<String> taskNames = taskSearch.map((task) => task.taskName).toList();

          print(taskNames); // This will print the list of task names
  ...
  userPunchIn(BuildContext context) async {
    try {
      ...
      if (decodedToken != null) {
        ...
        final response = await ApiRepository.to.punchIn(request: request);

        if (response.status == 200) {
          showSuccessSnackbar(
              title: 'Success', message: 'You have successfully Punched IN');
  ...
  userPunchOut(BuildContext context) async {
    try {
      ...
      if (decodedToken != null) {
        ...
        final response = await ApiRepository.to.punchOut(request: request);

        if (response.status == 200) {
          _punchOutMessage.value = response.data;
          showSuccessSnackbar(
              title: 'Success', message: 'You have successfully Punched OUT');
          await Future.delayed(const Duration(seconds: 4));
          // ignore: use_build_context_synchronously
          Navigator.pop(context);
        } else if (response.message == "Failed") {
